...
# lime green
local c14=$'\e[38;5;46m'
# white
local c15=$'\e[38;5;256m'
# ls blue
local c16=$'\e[38;5;12m'
...
prompt_jnrowe_precmd () {
  ...
  if [ "${vcs_info_msg_0_}" = "" ]; then
    dir_status="%{$c14%}%n%{$c4%} %{$(zsh_path)%} %{$c0%}(%{$c5%}%?%{$c0%})"
    PROMPT="
${dir_status} ${ret_status}%{$reset_color%}
> "
...
> "
    ...
    dir_status="%{$c14%}%n%{$c4%} %{$(zsh_path)%} %{$c0%}(%{$c5%}%?%{$c0%})"
    PROMPT="
${vcs_info_msg_0_}%{$bg_bold[red]%}%{$fg_bold[blue]%}D%{$fg_bold[black]%}IRTY%{$reset_color%}
${dir_status}%{$reset_color%}
%{$c13%}>%{$c0%} "
  else
    dir_status="%{$c14%}%n%{$c4%} %{$(zsh_path)%} %{$c0%}(%{$c5%}%?%{$c0%})"
...
> "
  ...
  if [ ! -z "${VIRTUAL_ENV}" ]; then
    PROMPT="
%{$c1%}VENV (%{$c4%}${VIRTUAL_ENV}%{$c1%})%{$reset_color%}$PROMPT"
  fi
